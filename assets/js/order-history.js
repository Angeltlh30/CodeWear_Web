document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 2. Gọi API lấy dữ liệu từ MongoDB
    try {
        const res = await fetch(`http://localhost:3000/api/orders/${user.id || user._id}`);
        ...
        if (orders.length === 0) {
            container.innerHTML = `
                <div style="text-align:center; padding:60px; background:#fff; border-radius:8px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                    <img src="../assets/image/coming_soon.png" style="width:100px; opacity:0.5; margin-bottom: 20px;">
        ...
        // Duyệt qua từng đơn hàng (Mới nhất lên đầu)
        orders.forEach(order => {
            
            // --- XỬ LÝ TRẠNG THÁI (Màu sắc & Size chữ) ---
            let statusStyle = 'color: #ee4d2d;'; // Mặc định màu ĐỎ (Đang xử lý, Đã hủy...)
            if (order.status === 'Đã hoàn thành' || order.status === 'Hoàn thành') {
                statusStyle = 'color: #26aa99;'; // Màu XANH LÁ
            }
                ...
                <div class="order-card" style="background: #fff; margin-bottom: 20px; border-radius: 4px; box-shadow: 0 1px 2px rgba(0,0,0,0.1);">
                    ...
                    <div class="card-header" style="padding: 15px 20px; border-bottom: 1px solid #eaeaea; display: flex; justify-content: space-between; align-items: center;">
                        <div class="shop-info" style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <span style="color: #888; font-size: 13px;">Ngày đặt: ${order.date}</span>
                        </div>
                    ...
                    <div class="card-footer" style="padding: 20px; background: #fffefb; border-top: 1px solid #f1f1f1; border-bottom-left-radius: 4px; border-bottom-right-radius: 4px;">
                        ...
                        <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 15px;">
                            <button style="min-width: 140px; padding: 10px 0; background: #ee4d2d; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px;">Mua lại</button>
                        </div>
                    </div>
                </div>
            `;
        ...
        container.innerHTML = '<p style="text-align:center; padding:40px; color:red;">Lỗi kết nối Server. Vui lòng thử lại sau.</p>';
    }
});